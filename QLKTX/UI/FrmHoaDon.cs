    ...
    public partial class FrmHoaDon : Form
    {
        ...
        public FrmHoaDon(string MaHD)
        {
            ...
            if (hoaDon != null)
            {
                ...
                try
                {
                    dtNgayHD.Value = DateTime.ParseExact(hoaDon.Rows[0]["NgayHD"].ToString(), "yyyy-MM-dd", CultureInfo.CurrentCulture);
                }
                catch { }
        ...
        private void InitKhu()
        {
            ...
            var dt = FrmMain.bS_Layer.Select_DSKhu(ref error);

            if (dt.DataSet == null) return;
        ...
        private void cmbKhu_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbKhu.SelectedIndex > 0)
            {
                ...
                var dt = FrmMain.bS_Layer.Select_DSMaPhong(ref error, cmbKhu.Text.Trim());
        ...
        private void btnHoanTat_Click(object sender, EventArgs e)
        {
            ...
            if (MaHD != "")
            {
                bool reHoaDon = FrmMain.bS_Layer.Update(ref error, MaHD, txtNamHoc.Text.Trim(), txtThang.Text.Trim(), dtNgayHD.Value, 
                                                    txtMaNV.Text.Trim(), cmbPhong.Text.Trim(), cmbKhu.Text.Trim());
                ...
                foreach (string maDV_key in DichVuSuDung.Keys)
                {
                    ...
                    bool exist = FrmMain.bS_Layer.Select_SDDV(ref error, MaHD, maDV_key);

                    //Nếu đã có, cập nhật nó
                    if (exist && error.Contains("tồn tại"))
            ...
            else
            {
                ...
                //Tạo mới 1 hóa đơn
                bool createHoaDon = FrmMain.bS_Layer.Insert(ref error, txtNamHoc.Text.Trim(), txtThang.Text.Trim(), dtNgayHD.Value,
                                                            FrmMain.MaNV, cmbPhong.Text.Trim(), cmbKhu.Text.Trim(), ref identity);
